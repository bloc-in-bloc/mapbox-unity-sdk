namespace Mapbox.Unity.Ar
{
	...
	public class SimpleAutomaticSynchronizationContextBehaviour : MonoBehaviour, ISynchronizationContext
	{
	 	...
	 	void Start()
		{
			...
			arPlaneManager.planesChanged += PlaneChangesHandler;
		}
		...
		void PlaneChangesHandler(ARPlanesChangedEventArgs eventArgs)
		{
			foreach( ARPlane arPlane in eventArgs.added) {
				_lastHeight = arPlane.center.y;
			}
		}
		...
		void LocationProvider_OnLocationUpdated(Location location)
		{
			...
			if (location.IsLocationUpdated || location.IsUserHeadingUpdated)
			{
				...
				deviceOrientation.text = $"DeviceOrientation: { location.DeviceOrientation }";
				userHeading.text = $"UserHeading: { location.UserHeading }";
